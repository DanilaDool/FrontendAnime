...
function AnimeListPage() {
    ...
    useEffect(() => {
        const fetchPage = async () => {
            ...
            setIsFetchingNextPage(currentPage > 1);

            try {
                const res = await axios.get(`http://localhost:3000/api/v1/anime/info_block?page=${currentPage}`);
                const data = res.data;

                if (Array.isArray(data.animes)) {
                    setAnimes(prev => {
                        const seen = new Set(prev.map(a => a.id));
                        const unique = data.animes.filter(a => !seen.has(a.id));
                        return [...prev, ...unique];
                    });
